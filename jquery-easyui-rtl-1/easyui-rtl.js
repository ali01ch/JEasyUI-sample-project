...
(function($){
	$.map(['validatebox','textbox','filebox','passwordbox','searchbox',
			...
			'datebox','datetimebox','numberbox','switchbutton',
	...
	function checkScrollLeftType(){
		var tmp = $('<div dir="rtl" style="font-size: 14px; width: 1px; height: 1px; position: absolute; top: -1000px; overflow: scroll">testtesttesttest</div>').appendTo('body');
		if (tmp.scrollLeft() > 0){
			$._rtlScrollType = 'default';	// chrome
		} else {
			tmp.scrollLeft(10);
			if (Math.floor(tmp.scrollLeft()) == 0){
